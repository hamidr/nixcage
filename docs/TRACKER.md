...
| [ADR-022](ADR-022-a-cage-is-bounded-by-what-the-host-declares.md) | A cage is bounded by what the host declares, and a session may ask for something else | proposed | -- | ADR-003, ADR-009, ADR-012, ADR-017, ADR-019 | nixcage.bounds and cages.<path>.bounds give memory and cpus to both substrates; the flag still wins |  |
