...
| [ADR-014](ADR-014-a-cage-without-the-daemon-sees-only-its-closure.md) | A cage without the daemon sees the closure of its roots, not the store | proposed | -- | ADR-009, ADR-011 | enter --no-nix-daemon binds each path of its roots' closure and nothing else of /nix/store; --store-root names a root |  |
