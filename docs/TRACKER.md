...
| [ADR-025](ADR-025-a-host-declares-the-binds-a-cage-always-has.md) | A host declares the binds a cage always has, and a session still asks for its own | proposed | -- | ADR-009, ADR-019, ADR-020, ADR-022, ADR-024 | nixcage.cages.<path>.binds renders a table the session adds to what it was asked for, with a clash refused |  |
